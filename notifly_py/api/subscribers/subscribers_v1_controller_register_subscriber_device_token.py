...
from ...models.register_subscriber_device_token_request_dto import RegisterSubscriberDeviceTokenRequestDto
...
def _get_kwargs(
    ...
    provider_id: str,
    *,
    body: RegisterSubscriberDeviceTokenRequestDto,
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/v1/subscribers/{subscriber_id}/credentials/{provider_id}/token".format(
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = SubscriberResponseDto.from_dict(response.json())
...
def sync_detailed(
    ...
    *,
    ...
    """Register a device token
     ...
     Idempotently register ONE **deviceToken** on the subscriber's **providerId** channel
          (e.g. **fcm**, **apns**). Re-registering the same token is a no-op (no duplicate). Pass
          **previousToken** to atomically swap a rotated token. On overflow the OLDEST tokens are
    dropped.
          Never wipes the channel — unlike a whole-array credentials update. The token travels in the
    body
          because it may contain URL-hostile characters.
